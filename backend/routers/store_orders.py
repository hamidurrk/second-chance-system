...
router = APIRouter(prefix="/api/store/orders")

@router.get("/")
async def get_store_orders(current_user: TokenData = Depends(is_admin_user)):
    store_id = current_user.store_id
    orders = read_records('order_response_view', conditions={'store_id': store_id})
    return orders
